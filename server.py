...
def token_encode(text: str):
    tokens = tokenizer.encode(
        text, add_special_tokens=False, add_prefix_space=True)
    return tokens
...
async def hello(clientsocket, path):
    ...
    if not started:
        ...
        print(
            "Starting model {} ({}-bit)".format(data["model"], data["bit_mode"]))
        ...
        if request == Request.QUIT:
            ...
            with torch.no_grad():
                for step in range(max_length):
                    ...
                    logits, next_pasts = model(
                        input_ids=input_ids_next, past_key_values=pasts)
                    logits.to("cuda")
                    ...
                    if generated_amount % 25 == 0:
                        print(
                            ("Generated {}/{}".format(generated_amount, max_length)))
